// index.js — Conektar S.A. • Bot de Cotizaciones + Costeo de Impo (ESM) • v3.0 (modificado con doble búsqueda y fixes)
...
/* ========= WEBHOOK ========= */
// Cache para evitar duplicados de mensajes (anti-duplicate for double greetings)
const messageCache = new Map(); // from: { msgId: timestamp }
...
app.post("/webhook", async (req,res)=>{
  try{
    ...
    const from  = msg.from;
    const msgId = msg.id; // Usar msg.id para chequear duplicados
    const cache = messageCache.get(from) || {};
    if (cache[msgId]) {
      console.log("Duplicado detectado para msgId:", msgId);
      return res.sendStatus(200); // Ignorar duplicado
    }
    cache[msgId] = Date.now();
    messageCache.set(from, cache);
    ...
    if (type==="text") {
      ...
      // Fix: Manejar texto "confirmar" como botón (para no atascarse)
      if (lower === "confirmar" && s.step !== "cotizar") {
        s.step = "cotizar";
      }
