// index.js — ESM ✅ Solo BOTONES — Logo inicial, tono cálido, match de aeropuertos tolerante
...
const LOGO_URL = String(process.env.LOGO_URL || "https://www.conektarsa.com/wp-content/uploads/2022/06/cropped-Logo-1-2048x521.png").trim();
...
async function cotizarAereo({ origen, kg, vol }) {
  ...
  // match si cualquiera de los tokens (ciudad o IATA) aparece en la celda Origen
    ...
    if (type === "interactive") {
      ...
      if (id === "aer_carga" || id === "aer_courier" || id === "aer_volver") {
        ...
        if (s.data.subtipo === "carga") {
          await sendText(from,"✈️ *Origen* (podés escribir *PVG*, *Shanghai*, *Tokio/Tokyo/NRT*, *Pekín/Beijing/PEK*, etc.)");
        } else {
    ...
    if (type === "text") {
      ...
      if (s.step === "origen") {
        ...
        if (s.data.tipo === "maritimo") {
          ...
          if (!r) { await sendText(from,"❌ No encontré esa *ruta/modalidad* en tu planilla (Marítimos). Probá con el nombre tal cual figura."); return res.sendStatus(200); }
          const resp =
`✅ *Tarifa estimada (Marítimo ${s.data.modalidad})*
USD ${fmt(r.totalUSD)} *todo-in freight* + *Gastos Locales*.
...
📧 comercial@conektarsa.com`;
          ...
          await logSolicitud([new Date().toISOString(), from, "", s.data.empresa, "whatsapp","maritimo", s.data.origen, r.destino, "", "", s.data.modalidad, r.totalUSD, `Marítimo ${s.data.modalidad} ${s.data.origen}→${r.destino}`]);
...
📧 comercial@conektarsa.com`;
          await sendText(from, resp);
          await logSolicitud([new Date().toISOString(), from, "", s.data.empresa, "whatsapp","terrestre", s.data.origen, r.destino, "", "", "", r.totalUSD, `Terrestre ${s.data.origen}→${r.destino}`]);
          sessions.delete(from);
          return res.sendStatus(200);
        }
      }
      ...
      if (s.step === "peso_vol") {
        ...
        if (!r) { await sendText(from,"❌ No encontré esa ruta en *Aéreos*. Probá con *ciudad* o *código IATA* (ej.: PVG, PEK, NRT)."); return res.sendStatus(200); }
...
✨ *Tu consulta ha sido registrada correctamente.*  
Nuestro equipo comercial te contactará pronto para darte
